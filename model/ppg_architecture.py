"""
Neural network architecture for PPG-only hypertension prediction.
...
Context (Sleep Stage ⊕ Apnea):
  - Concatenated with PPG instance embeddings per instance, then fused via MLP.
  - Positional encoding over instance (segment) dimension.

PRV (global context):
  - PPG uses PRV (Pulse Rate Variability), not HRV. PRV = ppg_hf, ppg_lf (from ppg_hrv group).
  - Project bag-level PRV → global_emb; broadcast to all instances; concat [h, global_emb] per instance.
  - MIL pooling runs on h_with_global; bag embedding = pooled over instances.

Expects PPG signal + PRV (ppg_hf, ppg_lf) + sleep stage + apnea. No ppg_ft.
"""
...
class AttentionPooling(nn.Module):
    ...
    Used in PPGBackbone (no statistics pooling).
    """
...
# ---------------------------------------------------------------------------
# PPGNet: PPG-only network mirroring FusionNet style
# ---------------------------------------------------------------------------
class PPGNet(nn.Module):
    """
    PPG-only network for hypertension prediction.
    Uses PPG signal + PRV (ppg_hf, ppg_lf) + sleep stage + apnea.
    PPG does not have HRV; it has PRV (Pulse Rate Variability) from ppg_hrv group.
    """
    def __init__(
        self,
        embed_dim: int = 64,
        ...
        # PPG encoder: multi-scale CNN + attention pooling (like ECG)
        self.ppg_encoder = PPGBackbone(1, embed_dim, dropout=0.3)

        # Context features concatenated with PPG instance embeddings
        ...
        # Fusion layer: PPG embed + context → embed_dim
        ...
        # PRV global projection (ppg_hf, ppg_lf - broadcast to instances before MIL pooling)
        prv_out_dim = 0
        if self.use_prv:
            self.prv_proj = nn.Sequential(
                nn.LayerNorm(prv_dim),
                nn.Linear(prv_dim, 16),
                nn.GELU(),
                nn.Dropout(0.3),
            )
            prv_out_dim = 16

        # MIL pooling (input includes broadcast PRV if used)
        instance_in_dim = embed_dim + prv_out_dim if self.use_prv else embed_dim
        if mil_pooling == 'topk':
        ...
        self.bag_classifier = nn.Linear(embed_dim + prv_out_dim, 1)
    ...
    def forward(self, ecg=None, ppg=None, mask=None, hrv=None,
        ...
        """
        Args:
            ppg:          (B, T, L_ppg) — PPG signal segments
            mask:         (B, T) — boolean mask for valid segments
            hrv:          (B, prv_dim) — bag-level PRV features (ppg_hf, ppg_lf); named hrv for API compatibility
        ...
        # Module I: instance embeddings from PPG backbone
        ppg_flat = ppg.view(B * T, 1, L_ppg)
        ppg_emb = self.ppg_encoder(ppg_flat).view(B, T, self.embed_dim)  # (B, T, embed_dim)
        ...
        # Fuse PPG + context → embed_dim
        fused = self.fusion(combined.view(-1, combined.shape[-1])).view(B, T, self.embed_dim)
        fused = self.fusion_ln(fused)
        # Positional encoding over instance dimension
        fused = fused + self.pos_encoding(fused)

        # Global PRV context: project and broadcast to all instances, then concatenate
        if self.use_prv:
            prv_emb = self.prv_proj(hrv) if hrv is not None else torch.zeros(B, 16, device=fused.device, dtype=fused.dtype)
            prv_broadcast = prv_emb.unsqueeze(1).expand(B, T, -1)       # (B, T, 16)
            h_with_global = torch.cat([fused, prv_broadcast], dim=-1)   # (B, T, embed_dim + 16)
        ...
        embeddings_before_classifier = bag_emb  # (B, embed_dim + prv_out_dim)
